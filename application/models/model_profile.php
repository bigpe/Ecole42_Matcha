...
class Model_Profile extends Model{
    ...
    public function get_data()
    {
        ...
        $data[0]['image_count'] = $db->db_read("SELECT COUNT(*) FROM USER_PHOTO WHERE user_id='$user_id'");
        $data[0]['tags'] = $db->db_read_multiple("SELECT tag_name FROM USERS_TAGS INNER JOIN TAGS on TAGS.tag_id=USERS_TAGS.tag_id WHERE user_id='$user_id'");
        $data[0]['user_pic'] = $db->db_read_multiple("SELECT photo_src FROM USER_PHOTO WHERE user_id = '$user_id' group by photo_src DESC;");
    ...
    public function change_tags($tags){
        ...
        $tag_id = $db->db_read("SELECT tag_id FROM Matcha.TAGS WHERE tag_name = '$tags';");
        if (!$tag_id){
            $db->db_change("INSERT INTO Matcha.TAGS (tag_name) VALUE ('$tags') ;");
            $tag_id = $db->db_read("SELECT user_id FROM Matcha.TAGS WHERE tag_name = '$tags';");
        }
    ...
    public function add_photo($user_pic)
    {
        ...
        $path = "./images/user_photo/$name.jpg";
        $db = new database();
        ...
        else
        {
            ...
            $db->db_change("INSERT INTO USER_PHOTO (photo_src, user_id) VALUE ('$path','$user_id');");
        }
    }
}

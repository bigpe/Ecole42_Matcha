<?php
class Model_History extends Model{
    function get_user_data($login)
    {
        $db = new database();
        $user_id = $db->db_read("SELECT user_id FROM USERS WHERE login='$login'");
        $history = $db->db_read_multiple("SELECT photo_src, USERS.login, HISTORY.update_date, HISTORY.action_icon
FROM HISTORY JOIN USERS on USERS.user_id=HISTORY.alfa_user_id
JOIN USER_MAIN_PHOTO UMP on USERS.user_id = UMP.user_id
JOIN USER_PHOTO UP on UMP.photo_id = UP.photo_id
WHERE omega_user_id='$user_id' order by HISTORY.update_date DESC");

        return $history;
    }
}
